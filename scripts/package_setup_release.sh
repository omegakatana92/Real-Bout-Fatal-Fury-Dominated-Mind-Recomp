...
#
# Writes: dist/ffdm-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix ffdm \
  --exe-name Fatal_Fury_Dominated_Mind__Recompiled \
  --display-name "Fatal-Fury-Dominated-Mind- Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Fatal-Fury-Dominated-Mind- disc" \
